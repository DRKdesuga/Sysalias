...
static int isabs(const char *p) { return p && p[0] == '/'; }

int fs_xdg_config_dir(char *out, size_t cap) {
  const char *xdg = getenv("XDG_CONFIG_HOME");
  if (xdg && *xdg) { if (strlen(xdg) + 1 > cap) return -1; strcpy(out, xdg); return 0; }
  const char *home = getenv("HOME");
  if (!home || !*home) return -1;
  size_t need = strlen(home) + strlen("/.config") + 1;
  if (need > cap) return -1;
  strcpy(out, home); strcat(out, "/.config");
  return 0;
}
...
int fs_mkdir_p(const char *path) {
  ...
  for (; *q; q++) {
    if (*q == '/') {
      *q = 0;
      if (strlen(p) && mkdir(p, 0700) && errno != EEXIST) { free(p); return -1; }
      *q = '/';
    }
  }
  if (mkdir(p, 0700) && errno != EEXIST) { free(p); return -1; }
  free(p);
  return 0;
}
...
int fs_atomic_write(const char *path, const char *data, size_t len) {
  ...
  if (lp + 5 >= sizeof(tmp)) return -1;
  memcpy(tmp, path, lp);
  memcpy(tmp + lp, ".tmp", 5);
  FILE *f = fopen(tmp, "wb");
  if (!f) return -1;
  size_t wr = fwrite(data, 1, len, f);
  if (wr != len) { fclose(f); unlink(tmp); return -1; }
  if (fflush(f)) { fclose(f); unlink(tmp); return -1; }
