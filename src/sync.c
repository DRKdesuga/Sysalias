...
static int insert_block(const char *rcfile, const char *block)
{
    ...
    free(data);
    if (fs_atomic_write(rcfile, out, outn))
    {
        free(out);
        return -1;
    }
...
int sync_run(void)
{
    ...
    if (rc_path("bash", p, sizeof(p)))
    {
        free(b1);
        free(b2);
        return 1;
    }
    if (insert_block(p, b1))
    {
        free(b1);
        free(b2);
        return 1;
    }
    if (rc_path("zsh", p, sizeof(p)))
    {
        free(b1);
        free(b2);
        return 1;
    }
    if (insert_block(p, b2))
    {
        free(b1);
        free(b2);
        return 1;
