...
int s_split_kv(const char *in, char **name, char **body) {
    if (!in || !name || !body) return -1;
    const char *eq = strchr(in, '=');
    if (!eq || eq == in || eq[1] == 0) return -1;
    size_t ln = (size_t)(eq - in);
    size_t lb = strlen(eq + 1);
    char *n = malloc(ln + 1);
    char *b = malloc(lb + 1);
    if (!n || !b) { free(n); free(b); return -1; }
    memcpy(n, in, ln); n[ln] = 0;
    memcpy(b, eq + 1, lb + 1);
    char *nt = s_trim(n);
    char *bt = s_trim(b);
    if (nt != n) memmove(n, nt, strlen(nt) + 1);
    if (bt != b) memmove(b, bt, strlen(bt) + 1);
    *name = n;
    *body = b;
    return 0;
}
