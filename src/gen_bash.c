...
static int cb_filter(const char *n, const char *b, const char *sh, void *ud)
{
    ...
    {
        if (!(strcmp(sh, "any") == 0 || strcmp(sh, "bash") == 0))
            return 0;
    }
    else
    {
        if (!(strcmp(sh, "any") == 0 || strcmp(sh, "zsh") == 0))
            return 0;
    }
    return sb_add(s, n, b);
}
...
int gen_generate_all(void)
{
    sb_t sb = { 0 };
    sb.want = "bash";
    if (registry_iterate(cb_filter, &sb))
    {
        free(sb.buf);
        return -1;
    }
    if (write_file("aliases.bash", sb.buf ? sb.buf : "", sb.len))
    {
        free(sb.buf);
        return -1;
    }
    ...
    if (write_file("aliases.zsh", sb.buf ? sb.buf : "", sb.len))
    {
        free(sb.buf);
        return -1;
    }
